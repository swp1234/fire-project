...
- Rule-based labels: IQ와 Zodiac 상세 화면은 미리 작성된 규칙 기반 메모임을 명시하고 AI·광고 잠금으로 표시하지 않아야 한다. 12개 locale의 JSON-실제 DOM 정확 일치, UTF-8 치환 문자 부재, Zodiac 로더의 번역 초기화 대기, 모바일 결과 흐름, IQ 20번 문항 직후 완료 동작을 함께 검증한다.

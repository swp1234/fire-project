...
- **Tier 1 High-RPM Organic Recovery & GSC Alignment (Waves 1~27)**:
  - Registered missing killer apps (`future-self` 178s, `stress-response`, `burnout-test`) in root sitemap.
  - Lifted `noindex` on 52 Tier 1 guides (Gaslighting Recovery, Trauma Bonding, Dissociation Symptoms, Hypervigilance Trauma, People Pleasing Fawn, Somatic Anxiety, Sensory Overload HSP, 4 Attachment Styles EN/KO).
  - Upgraded Quick Rails to point directly to top killer tools (`attachment-style`, `stress-check`, `stress-response`, `brain-type`).
- **Verification**: Full test suite PASS (Analytics 9/9, Runtime 6/6, Indexing Inventory 127 clean, 0 errors).

...
- **Tier 1 High-RPM Organic Recovery & GSC Alignment (Waves 1~28)**:
  - Registered missing killer apps (`future-self` 178s, `stress-response`, `burnout-test`) in root sitemap.
  - Lifted `noindex` on 54 Tier 1 guides (Complex PTSD Recovery, Gaslighting, Trauma Bonding, Dissociation, Hypervigilance, Fawn, Somatic Anxiety, Sensory Overload, 4 Attachment Styles EN/KO).
  - Upgraded Quick Rails to point directly to top killer tools (`stress-response`, `stress-check`, `attachment-style`, `brain-type`).
- **Verification**: Full test suite PASS (Analytics 9/9, Runtime 6/6, Indexing Inventory 129 clean, 0 errors).

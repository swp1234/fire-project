...
- **Tier 1 High-RPM Organic Recovery & GSC Alignment (Waves 1~91)**:
  - Registered missing killer apps (`future-self` 178s, `stress-response`, `burnout-test`) in root sitemap.
  - Lifted `noindex` on 180 Tier 1 guides (Toxic Positivity, Nervous Breakdown EN/KO).
  - Upgraded Quick Rails to point directly to top killer tools (`iq-test`, `eq-test`, `brain-type`, `stress-check`).
- **Verification**: Full test suite PASS (Analytics 9/9, Runtime 6/6, Indexing Inventory 255 clean, 0 errors).

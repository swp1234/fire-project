...
const APP = path.join(ROOT, 'projects', 'trauma-response');
const LOCALES = ['ko','en','ja','zh','es','pt','id','tr','de','fr','hi','ru'];
const EVENTS = ['trauma_response_view','trauma_response_start','trauma_response_progress','trauma_response_complete','trauma_response_next_click','trauma_response_share','trauma_response_related_click'];
...
function verifySource(overrides = {}) {
  ...
  assert(/href="\/stress-check\/\?lang=ko&amp;source=trauma_response_result"/.test(value.html), 'Primary Stress Check action drifted');
  ...
  assert(/this\.answers\.length === 4\) this\.trackStage\('trauma_response_progress'\)/.test(value.app), 'Progress must require four accepted answers');
  assert(!/trackStage\('trauma_response_[^']+'\s*,/.test(value.app), 'Private value entered funnel telemetry');
  ...
  assert(/CACHE_NAME = 'trauma-response-v2'/.test(value.sw), 'Service worker cache version drifted');
  ...
  assert(manifest.scope === '/trauma-response/' && manifest.start_url === '/trauma-response/', 'Manifest scope drifted');
...
async function startServer() {
  ...
  const server = http.createServer((request, response) => {
    try {
      ...
      if (!pathname.startsWith('/trauma-response/')) return response.writeHead(404).end();
      const relative = pathname.slice('/trauma-response/'.length) || 'index.html';
...
async function verifyRuntime(baseUrl) {
  ...
  try {
    for (const test of [{ width:390, height:844, lang:'en' }, { width:1440, height:900, lang:'fr' }]) {
      ...
      await page.goto(`${baseUrl}/trauma-response/?lang=${test.lang}`, { waitUntil:'domcontentloaded', timeout:20000 });
      await page.waitForFunction(() => window.traumaResponseApp && getComputedStyle(document.getElementById('app-loader')).display === 'none', null, { timeout:15000 });
      ...
      assert(await page.evaluate(() => !(window.dataLayer || []).some(row => row?.[1] === 'trauma_response_start')), `${test.width}px load counted as start`);
      ...
      const expectedHref = `/stress-check/?lang=${test.lang}&source=trauma_response_result`;
...
function verifyMutations() {
  ...
  const cases = [
    ...
    ['private-event', { app:base.app.replace("this.trackStage('trauma_response_complete');", "this.trackStage('trauma_response_complete', this.resultType.id);") }],
    ...
    ['primary-route', { html:base.html.replace('/stress-check/?lang=ko&amp;source=trauma_response_result','/portal/') }],
    ...
    ['event', { app:base.app.replace("this.trackStage('trauma_response_start');", '') }],
    ...
    ['manifest', { manifest:base.manifest.replace('"scope": "/trauma-response/"','"scope": "/"') }],
    ['locale-boundary', { locales:{ ...base.locales, en:base.locales.en.replace('This author-created activity is for private reflection. It does not diagnose trauma, PTSD, or any health condition.','') } }],
...
async function main() {
  ...
  console.log(`[PASS] Trauma Response containment: ${result.locales} locales, ${result.related} focused routes, ${result.events} private stages`);
}

...
const APP = path.join(ROOT, 'projects', 'sleep-animal');
const TARGET = path.join(ROOT, 'projects', 'animal-personality');
const PORTAL = path.join(ROOT, 'projects', 'portal');
const OLD_PATH = '/sleep-animal/';
const TARGET_PATH = '/animal-personality/';
...
function verifySource(overrides = {}) {
  ...
  assert(!/googletagmanager|pagead2|adsbygoogle|application\/ld\+json|FAQPage|aggregateRating|page_engage|sleep_type/i.test(html), 'Retired route contains ads, analytics, schema, or fake measurement');
  assert(/startsWith\('sleep-animal'\)/.test(worker) && /caches\.delete\(name\)/.test(worker), 'Old caches are not narrowly removed');
  ...
  const stale = sources.filter(([, source]) => /\/sleep-animal\/|appId:\s*['"]sleep-animal|id:\s*['"]sleep-animal/i.test(source));
...
function mutations() {
  ...
  const cases = [
    ...
    ['fake-measurement', { html: baseline.html.replace('</main>', '<p>sleep_type score 95%</p></main>') }],
    ['bundle', { files: [...baseline.files, 'js/app.js'].sort() }],
    ['cache', { worker: baseline.worker.replace("name.startsWith('sleep-animal')", 'name.length > 0') }],
    ['worker', { worker: baseline.worker.replace('await self.registration.unregister();', '') }],
    ['promotion', { sources: [...baseline.sources, [path.join(PORTAL, 'fake.html'), '<a href="/sleep-animal/">old</a>']] }],
  ];
...
async function localServer() {
  ...
  const instance = http.createServer((request, response) => {
    try {
      ...
      const type = path.extname(target) === '.css' ? 'text/css' : path.extname(target) === '.js' ? 'application/javascript' : path.extname(target) === '.json' ? 'application/json' : 'text/html; charset=utf-8';
...
async function redirect(origin) {
  ...
  try {
    for (const width of [390, 1440]) {
      ...
      const offenders = overflow > 0 ? await page.evaluate(() => [...document.querySelectorAll('body *')].map((element) => {
        const rect = element.getBoundingClientRect();
        const ancestors = [];
        for (let parent = element.parentElement; parent && ancestors.length < 4; parent = parent.parentElement) {
          const parentRect = parent.getBoundingClientRect();
          ancestors.push(`${parent.tagName}.${String(parent.className || '').slice(0, 30)}:${Math.round(parentRect.width)}`);
        }
        return { tag: element.tagName, id: element.id, className: String(element.className || '').slice(0, 80), left: Math.round(rect.left), right: Math.round(rect.right), width: Math.round(rect.width), ancestors };
      }).filter((row) => row.right > innerWidth + 1 || row.left < -1).slice(0, 8)) : [];
      assert(overflow <= 0, `${width}px destination overflow: ${overflow}px; ${JSON.stringify(offenders)}`);
      await page.close();
    }
...
async function productionParity(origin) {
  const checks = [
    ...
    [PORTAL, 'blog/en/animal-personality-test-discover.html', '/portal/blog/en/animal-personality-test-discover.html'],
    [TARGET, 'index.html', TARGET_PATH], [TARGET, 'js/app.js', `${TARGET_PATH}js/app.js`],
...
async function main() {
  ...
  if (index >= 0) {
    ...
    console.log(`[PASS] Sleep Animal retirement production: ${result.bytes} bytes, ${result.files} files -> ${TARGET_PATH}`);
    return;
  }
  const local = await localServer();
  try { await redirect(local.origin); } finally { await local.close(); }
  console.log(`[PASS] Sleep Animal retirement local: ${result.bytes} bytes, ${result.files} files -> ${TARGET_PATH}`);
}
